...
def reserved():
    RSVD = time.time()
    print 'Starting RSVD:', multiprocessing.current_process().name
    startrsvd = time.time()
    dicttriyrnoup = []
    dictyrnoup = []
    dictyrpartup = []
    ...
    with open('/tmp/res-prices.json', "w") as f:
        ...
        print("RSVD done")
        endrsvd = time.time()
        print'RSVD :', (endrsvd - RSVD)
        print 'Exiting RSVC:', multiprocessing.current_process().name
        end = time.time()
        print(end - RSVD)
...
def ondemand():
    ondemandstart = time.time()
    print 'Starting ondemand:', multiprocessing.current_process().name
    startod = time.time()
    oddicts = []
    try:
        for y in prodf:
        ...
        print'ondemand :', (end - ondemandstart)
        print 'Exiting ondemand :', multiprocessing.current_process().name
...
# def parse_ec2_terms():
def parse_ec2_terms(event, context):
    # try:
    ...
    p2.join()
    while False:
        odfile = ("/temp/odprices.json")
        rsvdfile = ("/temp/res-prices.json")
        try:
            if os.path.isfile(odfile) and os.path.isfile(rsvdfile):
                time.sleep(3)
        finally:
            mergeprices()
# parse_ec2_terms()
